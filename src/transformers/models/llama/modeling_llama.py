...
class AsymmetricLlamaMLP(nn.Module):
    def __init__(self, config, lit_mlp_config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size

        self.lit_mlp_config = lit_mlp_config
        ...
        # )
        self.gate_proj = nn.Linear(
            self.lit_mlp_config.n_embd_mlp_in,
            self.lit_mlp_config.intermediate_size,
        ...
        self.down_proj = nn.Linear(
            self.lit_mlp_config.intermediate_size,
            self.lit_mlp_config.n_embd_mlp_out,
            bias=config.mlp_bias,
        )
        self.act_fn = ACT2FN[config.hidden_act]
